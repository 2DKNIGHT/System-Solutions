    ...
    public class SystemSolution
    {
        ...
        public double[] RungeKutta(double x, double y, double z, double tau, double h, double z1)
        {
            ...
            for (int i = 0; i < k1.Length; i++)
            {
                ...
                k2[i] = h * getXfunction(_x);
                k1[i] = h * getYfunction(_y);
                k4[i] = h * getZ1function(_z1, _z, _x);
                k3[i] = h * getZfunction(_z1, _z, _tau);
            }
        ...
        public double getZfunction(double z1, double z, double tau)
        {
            if (Score  == 0)
            {
            ...
            if (Score == 1)
            {
                return -_ro - (_mu * Math.Cos(tau)) - (_alpha0 * _alpha0) * (z - _X10 - _eta);
            }
            if (Score == 2)
            {
                return -_ro - (_mu * Math.Cos(tau)) - (_alpha0 * _alpha0) * (z - z1 - _eta);
            }
            return 0;
        }
        public double getZ1function(double z1, double z, double x)
        {
            if (Score == 2)
            {
                return -(_alpha * _alpha) * (z1 - z + _eta) - _mu0 * _fi * Math.Sign(x) - _ro + _q;
            }
            return 0;
        }
        public double getYfunction(double y)
        {
            return y;
        }
        public double getXfunction(double x)
        {
            return x;
        }
    }
